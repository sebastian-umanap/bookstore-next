...
export default function CrearAutorPage() {
  ...
  const todayISO = useMemo(() => new Date().toISOString().slice(0, 10), []); //memo para que no cambie en cada render
  ...
  function validate(values: { name: string; description: string; birthDate: string; image: string }): Errors {
    ...
    const urlOk = /^https?:\/\/.+/i.test(image); //expresiones regulares para validar que inicie con http o https, sirven los dos casos.
    const extOk = /\.(png|jpe?g|webp|gif|bmp|svg)(\?.*)?$/i.test(image); //experesiones regulares para validar el tipo de archivo en la url
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const values = {
      name: String(data.get("name") || "").trim(), //trim para eliminar espacios al inicio y final
      description: String(data.get("description") || "").trim(), //trim para eliminar espacios al inicio y final
      birthDate: String(data.get("birthDate") || ""),
      image: String(data.get("image") || "").trim(), //trim para eliminar espacios al inicio y final
    };

    const eObj = validate(values); //diferentes validaciones se van a hacer
  ...
  const inputClass =
    ...
    "dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder:text-zinc-400 dark:border-zinc-700"; //se crea clase de tailwind para los inputs y mantener la misma estetica en todos los inputs
